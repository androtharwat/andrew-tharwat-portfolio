(() => {
  ...
  style.textContent = `
    /* Temporarily disable project-card navigation until detail pages are ready */
    ...
    /* Important CTA immediately after projects */
    ...
    .after-work-contact-inner{position:relative;z-index:1;display:grid;grid-template-columns:1fr auto;gap:28px;align-items:center}
    .after-work-contact .micro{display:block;color:#ff4050;font-size:9px;font-weight:900;letter-spacing:2px;margin-bottom:8px}
    .after-work-contact h3{margin:0;font-size:clamp(24px,3vw,40px);line-height:1.02;letter-spacing:-1px}
    .after-work-contact h3 b{color:#ff3043}
    .after-work-contact p{margin:9px 0 0;color:#9fb2be;font-size:11px;line-height:1.65;max-width:720px}
    .after-work-actions{display:flex;gap:10px;flex-wrap:wrap;justify-content:flex-end}
    .after-work-actions a{display:inline-flex;align-items:center;gap:9px;text-decoration:none;padding:14px 17px;border-radius:12px;font-size:10px;font-weight:900;white-space:nowrap;transition:transform .2s ease,box-shadow .2s ease}
    .after-work-actions a:hover{transform:translateY(-2px)}
    .after-work-wa{background:linear-gradient(135deg,#e10613,#ff3043);color:#fff;box-shadow:0 14px 34px rgba(225,6,19,.24)}
    .after-work-mail{border:1px solid rgba(255,255,255,.18);background:rgba(255,255,255,.055);color:#fff}

    /* Main contact / collaboration section */
    ...
    /* Mobile contact must always be visible and easy to reach */
    .mobile-contact-dock{display:none}
    @media(max-width:900px){.contact-heading,.contact-network{grid-template-columns:1fr;gap:30px}.contact-choice{max-width:720px}.contact-shell{padding:70px 0}.contact-heading h2{letter-spacing:-2px}.after-work-contact-inner{grid-template-columns:1fr}.after-work-actions{justify-content:flex-start}}
    @media(max-width:820px){
      .site-header{grid-template-columns:auto 42px auto!important;padding-left:14px!important;padding-right:14px!important;gap:10px!important}
      .site-header .brand{min-width:0}.site-header .brand img{width:42px!important;height:42px!important}.site-header .brand-copy{display:none!important}
      .site-header .menu-btn{display:block!important;justify-self:center}
      .site-header .header-cta{display:inline-flex!important;visibility:visible!important;opacity:1!important;justify-self:end;padding:10px 12px!important;font-size:9px!important;border-radius:10px!important;position:static!important;transform:none!important}
      .work-nav .btn{display:inline-flex!important;padding:10px 12px!important;font-size:9px!important}
      body{padding-bottom:72px}
      .mobile-contact-dock{position:fixed;z-index:999;left:10px;right:10px;bottom:calc(10px + env(safe-area-inset-bottom));display:grid;grid-template-columns:1.18fr .82fr;gap:8px;padding:8px;border-radius:17px;background:rgba(4,16,26,.92);border:1px solid rgba(255,255,255,.14);backdrop-filter:blur(18px);box-shadow:0 18px 48px rgba(0,0,0,.38)}
      .mobile-contact-dock a{min-height:46px;border-radius:12px;text-decoration:none;display:flex;align-items:center;justify-content:center;gap:8px;font-size:10px;font-weight:900;letter-spacing:.25px}
      .mobile-contact-dock .dock-wa{background:linear-gradient(135deg,#e10613,#ff3043);color:#fff}.mobile-contact-dock .dock-mail{background:rgba(255,255,255,.07);border:1px solid rgba(255,255,255,.12);color:#fff}
    }
    @media(max-width:620px){.contact-choice,.contact-social-grid{grid-template-columns:1fr}.contact-shell{padding:58px 0}.contact-heading h2{font-size:42px}.contact-heading{padding-bottom:26px}.contact-network{margin-top:28px}.contact-signoff{align-items:flex-start;flex-direction:column}.contact-signoff span{text-align:left}.after-work-contact{padding:28px 0}.after-work-actions{display:grid;grid-template-columns:1fr 1fr;width:100%}.after-work-actions a{justify-content:center;padding:13px 10px}.after-work-contact p{font-size:10px}}
    @media(max-width:390px){.site-header .header-cta{font-size:0!important;padding:10px 11px!important}.site-header .header-cta:before{content:"TALK";font-size:9px}.after-work-actions{grid-template-columns:1fr}.mobile-contact-dock{grid-template-columns:1fr 1fr}}
  `;
  ...
  function insertAfterProjects(){
    ...
    const anchor=document.querySelector('#explore') || document.querySelector('.work-browser');
    if(!anchor) return;
    const section=document.createElement('section');
    section.className='after-work-contact';
    section.setAttribute('aria-label','Start a project with Andrew Tharwat');
    section.innerHTML=`<div class="container after-work-contact-inner"><div><span class="micro">SEEN SOMETHING THAT SPARKED AN IDEA?</span><h3>DON'T JUST BROWSE. <b>BUILD YOURS.</b></h3><p>If one of the projects gave you an idea, contact me directly and tell me what you want to create. A rough idea is enough to start.</p></div><div class="after-work-actions"><a class="after-work-wa" href="${waHref}" target="_blank" rel="noopener">WHATSAPP <span>→</span></a><a class="after-work-mail" href="${mailHref}">EMAIL BRIEF <span>→</span></a></div></div>`;
  ...
  function createMobileDock(){
    ...
    dock.setAttribute('aria-label','Direct contact');
    dock.innerHTML=`<a class="dock-wa" href="${waHref}" target="_blank" rel="noopener">WA · START IDEA</a><a class="dock-mail" href="${mailHref}">@ · EMAIL</a>`;
    document.body.appendChild(dock);
  }

  function upgradeHeaderCTA(){
    const cta=document.querySelector('.site-header .header-cta');
    if(cta){cta.textContent="Let's Talk →";cta.href='#contact';cta.setAttribute('aria-label','Contact Andrew Tharwat');}
  }
  ...
  function upgradeContact(){
    ...
    container.innerHTML=`
      <div class="contact-heading">
        <div><span class="section-kicker">LET'S BUILD SOMETHING REAL</span><h2>HAVE A PROJECT?<br><b>OR JUST AN IDEA?</b></h2></div>
      ...
      <div class="contact-choice" aria-label="Choose how to contact Andrew">
        <a class="contact-wa" href="${waHref}" target="_blank" rel="noopener" data-contact="whatsapp"><span class="contact-icon">WA</span><span class="contact-copy"><strong>START ON WHATSAPP</strong><small>Fastest way to discuss your idea</small></span><span class="arrow">→</span></a>
        <a class="contact-mail" href="${mailHref}" data-contact="email-cta"><span class="contact-icon">@</span><span class="contact-copy"><strong>SEND PROJECT BY EMAIL</strong><small>Best for briefs, details and files</small></span><span class="arrow">→</span></a>
      </div>
      <div class="contact-network"><div><div class="contact-network-title">FIND MY WORK & PROJECTS</div><div class="contact-social-grid">
      ...
      </div></div><div class="contact-direct-box"><div class="contact-network-title">DIRECT CONTACT</div><h3>Choose what feels easier.</h3><p>You can start with one message. No formal brief is required — explain the idea in your own words and we'll build from there.</p><div class="contact-direct-row"><a href="${waHref}" target="_blank" rel="noopener"><b>WhatsApp</b><span>+20 150 541 4444 →</span></a><a href="${mailHref}"><b>Email</b><span>${email} →</span></a></div></div></div>
  ...
  createMobileDock();
  upgradeContact();
  setTimeout(insertAfterProjects,800);
})();
