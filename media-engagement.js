(()=>{
  ...
  const copy=()=>lang()==='ar'?{
    ...
    shareLabel:'شارك هذا المحتوى',nativeShare:'مشاركة / ستوري',facebook:'فيسبوك',whatsapp:'واتساب',linkedin:'لينكدإن',copyLink:'نسخ الرابط',copied:'تم نسخ رابط المحتوى ✓',shareHint:'الرابط يفتح نفس المحتوى مباشرة داخل موقع Andrew Tharwat.',shareFallback:'تم نسخ الرابط — افتح التطبيق والصقه في الستوري أو المنشور.'
    ...
    shareLabel:'SHARE THIS CONTENT',nativeShare:'SHARE / STORY',facebook:'Facebook',whatsapp:'WhatsApp',linkedin:'LinkedIn',copyLink:'Copy link',copied:'Content link copied ✓',shareHint:'The link opens this exact content on Andrew Tharwat Portfolio.',shareFallback:'Link copied — paste it into your story or post.'
  };
  ...
  function previewUrl(m){return `${SITE}/api/share?media=${encodeURIComponent(m.id)}`}
  ...
  async function workingShareUrl(m){const rich=previewUrl(m);try{const r=await fetch(rich,{method:'GET',cache:'no-store',headers:{'X-Share-Probe':'1'}});if(r.ok)return rich}catch(_e){}return directUrl(m)}
  function wireShare(panel,m,index){const status=panel.querySelector('.media-share-status');panel.querySelectorAll('[data-share-action]').forEach(btn=>btn.addEventListener('click',async()=>{const action=btn.dataset.shareAction,text=shareText(m,index),title=shareTitle(m,index),direct=directUrl(m);status.textContent='';if(action==='copy'){await copyShare(direct,status,copy().copied);return}if(action==='native'){const url=direct;if(navigator.share){try{await navigator.share({title,text,url});return}catch(err){if(err?.name==='AbortError')return}}await copyShare(url,status,copy().shareFallback);return}if(action==='whatsapp'){openShare(`https://api.whatsapp.com/send?text=${encodeURIComponent(`${text}\n\n${direct}`)}`,'whatsapp-share');return}const url=await workingShareUrl(m);if(action==='facebook'){openShare(`https://www.facebook.com/sharer/sharer.php?u=${encodeURIComponent(url)}&quote=${encodeURIComponent(title)}`,'facebook-share');return}if(action==='linkedin'){openShare(`https://www.linkedin.com/sharing/share-offsite/?url=${encodeURIComponent(url)}`,'linkedin-share')}}))}
