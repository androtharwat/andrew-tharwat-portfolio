(() => {
  ...
  async function loadSettings(){
    ...
    const settings = Object.fromEntries(data.map(r => [r.key, r.value || {}]));const hero=settings.hero||{}, about=settings.about||{}, stats=settings.stats||{}, contact=settings.contact||{};
    if(hero.eyebrow) setText('hero-eyebrow',hero.eyebrow);
    if(hero.title){const h=document.getElementById('hero-title');if(h){const parts=String(hero.title).split(/SYSTEMS, STORIES & VISUAL EXPERIENCES\.?/i);if(parts.length>1) h.innerHTML=`${escapeHtml(parts[0])}<span>SYSTEMS, STORIES &amp;</span> VISUAL EXPERIENCES.`;else h.textContent=hero.title;}}
    if(hero.subtitle) setText('hero-subtitle',hero.subtitle);
    if(about.title){const a=document.getElementById('about-title');if(a){ const bits=String(about.title).split('. '); a.innerHTML=`${escapeHtml(bits[0]||about.title)}.<br><b>${escapeHtml((bits.slice(1).join('. ')||'MANY WAYS TO CREATE.'))}</b>`; }}
  ...
  async function loadFeatured(){
    ...
    const { data, error } = await sb.from('portfolio_projects').select('id,title,slug,excerpt,cover_url,tags,portfolio_categories(name,color)').eq('status','published').eq('featured',true).order('sort_order',{ascending:true}).limit(5);
    if(error || !data || !data.length) return;
    host.innerHTML=data.map((p,i)=>{const cat=p.portfolio_categories?.name||'Project';const tags=(p.tags||[]).slice(0,3).join(' • ');const ss=spriteStyle(p.slug);return `<a class="project-live-card reveal visible delay-${Math.min(i,4)}" href="/projects/${encodeURIComponent(p.slug)}">${ss?`<div class="project-live-sprite" style="${ss}"></div>`:`<img src="${escapeHtml(resolveMedia(p.cover_url))}" alt="${escapeHtml(p.title)}" loading="lazy" />`}<div class="project-live-overlay"><span>${escapeHtml(cat)}</span><h3>${escapeHtml(p.title)}</h3><p>${escapeHtml(p.excerpt||tags)}</p><b>VIEW PROJECT →</b></div></a>`;}).join('');
  }
