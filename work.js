(() => {
  function ensureVisuals(next){
    if(!document.querySelector('link[href*="visual-overrides.css"]')){const l=document.createElement('link');l.rel='stylesheet';l.href='/visual-overrides.css?v=1';document.head.appendChild(l)}
  ...
  ensureVisuals(()=>{
    ...
    const logo=document.querySelector('.work-nav .brand img');if(logo)logo.src='/assets/logo-mark.png';
    const sb=window.supabase.createClient(cfg.supabaseUrl,cfg.supabaseKey);
    const esc=(v='')=>String(v).replace(/[&<>'"]/g,c=>({'&':'&amp;','<':'&lt;','>':'&gt;',"'":'&#039;','"':'&quot;'}[c]));
    const media=(u='')=>{if(!u)return'/assets/logo-mark.png';if(/^https?:\/\//i.test(u))return u;return '/'+u.replace(/^\//,'')};
    ...
    function render(){
      ...
      grid.innerHTML=rows.length?rows.map(p=>{const ss=spriteStyle(p.slug);return`<a class="work-card" href="/projects/${encodeURIComponent(p.slug)}"><div class="work-card-media">${ss?`<div class="work-card-sprite" style="${ss}"></div>`:`<img src="${esc(media(p.cover_url))}" alt="${esc(p.title)}" loading="lazy">`}<span class="work-card-cat">${esc(p.portfolio_categories?.name||'Project')}</span></div><div class="work-card-body"><h2>${esc(p.title)}</h2><p>${esc(p.excerpt||p.description||'')}</p><div class="work-tags">${(p.tags||[]).slice(0,4).map(t=>`<span>${esc(t)}</span>`).join('')}</div><div class="work-card-link">VIEW CASE STUDY →</div></div></a>`}).join(''):`<div class="work-empty">No projects match this filter yet.</div>`;
    }
    ...
    const interaction=document.createElement('script');interaction.src='/interaction-contact-v2.js?v=1';document.head.appendChild(interaction);
    load();
  });
})();