(() => {
  const authCard=document.querySelector('#auth-view .auth-card');
  if(authCard && !document.querySelector('#device-start')){
    authCard.classList.add('device-card');
    authCard.innerHTML=`<img src="/assets/logo-mark.svg" alt="Andrew Tharwat" /><span>ANDREW THARWAT</span><h1>CONTROL CENTER.</h1><p id="device-message">No email. No password. Approve this browser once and it will open the Control Center automatically every time.</p><div id="device-start"><button id="trust-device" class="primary device-primary">TRUST THIS DEVICE</button><small>Your browser will create a private device key that stays only on this device.</small></div><div id="pairing-box" class="pairing-box hidden"><span>DEVICE APPROVAL CODE</span><strong id="pairing-code">------</strong><p>Send this 6-digit code to me here in ChatGPT. Once I approve it, this page will open automatically.</p><div class="pairing-status"><i></i><b>Waiting for approval…</b></div></div>`;
  }
  const forgetBtn=document.getElementById('sign-out'); if(forgetBtn) forgetBtn.textContent='Forget This Device';
