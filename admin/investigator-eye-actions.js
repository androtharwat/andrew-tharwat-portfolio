(() => {
  ...
  async function loadWorkflow(caseId){
    ...
    holder.innerHTML=`<div class="ie-admin-workflow-head"><h3>Case Workflow</h3><small>One screen · client sees one next action</small></div><div class="ie-admin-grid">
      ...
      <section class="ie-admin-block wide"><span>ADD RECOMMENDATION</span><div class="ie-admin-row"><select id="ie-rec-service"><option value="">Choose service</option>${available.map(s=>`<option value="${s.id}" data-mode="${s.pricing_mode}" data-start="${s.display_price_usd??''}">${esc(s.name_en)} · ${s.pricing_mode==='quote'?'Quote':s.pricing_mode==='from'?`From ${money(s.display_price_usd)}`:money(s.display_price_usd)}</option>`).join('')}</select><input id="ie-rec-price" type="number" min="0" step="5" placeholder="Exact quote USD · optional" /></div><div class="ie-admin-row"><textarea id="ie-rec-reason" placeholder="Why this service is recommended for this case…"></textarea><button class="primary" type="button" data-ie-add-recommendation>ADD RECOMMENDATION</button></div><p class="ie-admin-note">Fixed-price services use their catalog price automatically. For “From” or custom services, leave price empty to request an exact quote or enter the agreed exact price.</p><div class="ie-admin-existing">${caseServices.length?caseServices.map(x=>`<div><span><b>${esc(x.studio_service_catalog?.name_en||'Service')}</b><small>${esc(x.recommendation_reason||'No reason added')} · ${esc(x.status.replaceAll('_',' '))}</small></span><strong>${x.quoted_price!=null?money(x.quoted_price,x.currency||'USD'):'Quote pending'}</strong></div>`).join(''):'<div><span><b>No recommendations yet.</b><small>Add only what this case actually needs.</small></span></div>'}</div></section>
