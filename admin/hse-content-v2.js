(()=>{
  ...
  function fillKey(obj,key,value,locked,lockKey=key){
    if(!obj||locked.has(lockKey)||!isBlank(obj[key])||isBlank(value))return;
    obj[key]=value;
  }
  function deepLanguageFill(current,suggestion,lang){
    if(!current||!suggestion)return;
    if(Array.isArray(current)&&Array.isArray(suggestion)){
      current.forEach((item,i)=>{
        if(!item||typeof item!=='object')return;
        const key=item.key||item.id||item.when||item.section_key;
        const match=key?suggestion.find(x=>x&&typeof x==='object'&&(x.key===key||x.id===key||x.when===key||x.section_key===key)):suggestion[i];
        if(match)deepLanguageFill(item,match,lang);
      });
      return;
    }
    if(typeof current!=='object'||typeof suggestion!=='object')return;
    Object.keys(suggestion).forEach(key=>{
      const languageKey=lang==='ar'?(key==='ar'||key.endsWith('_ar')):(key==='en'||key.endsWith('_en'));
      if(languageKey){
        if(isBlank(current[key])&&!isBlank(suggestion[key]))current[key]=suggestion[key];
      }else if(current[key]&&suggestion[key]&&typeof current[key]==='object'&&typeof suggestion[key]==='object'){
        deepLanguageFill(current[key],suggestion[key],lang);
      }
    });
  }
  function mergeMissingLanguage(current,suggestion,lang){
    const c=JSON.parse(JSON.stringify(current)),d=suggestion||{},locked=new Set(c.locked_fields||[]);
    if(lang==='ar'){
      fillKey(c,'title_ar',d.title_ar,locked);
      fillKey(c,'excerpt_ar',d.excerpt_ar,locked);
      if(!locked.has('category')){c.category=c.category||{};fillKey(c.category,'name_ar',d.category?.name_ar,new Set());}
      if(!locked.has('hero')){c.hero=c.hero||{};fillKey(c.hero,'headline_ar',d.hero?.headline_ar,new Set());fillKey(c.hero,'subheadline_ar',d.hero?.subheadline_ar,new Set());}
      if(!locked.has('card')){c.card=c.card||{};fillKey(c.card,'description_ar',d.card?.description_ar,new Set());}
      if(!locked.has('seo')){c.seo=c.seo||{};fillKey(c.seo,'meta_title_ar',d.seo?.meta_title_ar,new Set());fillKey(c.seo,'meta_description_ar',d.seo?.meta_description_ar,new Set());}
    }else{
      fillKey(c,'title',d.title,locked);
      fillKey(c,'excerpt',d.excerpt,locked);
      if(!locked.has('category')){c.category=c.category||{};fillKey(c.category,'name',d.category?.name,new Set());}
      if(!locked.has('hero')){c.hero=c.hero||{};fillKey(c.hero,'headline',d.hero?.headline,new Set());fillKey(c.hero,'subheadline',d.hero?.subheadline,new Set());}
      if(!locked.has('card')){c.card=c.card||{};fillKey(c.card,'description',d.card?.description,new Set());}
      if(!locked.has('seo')){c.seo=c.seo||{};fillKey(c.seo,'meta_title',d.seo?.meta_title,new Set());fillKey(c.seo,'meta_description',d.seo?.meta_description,new Set());}
    }
    if(!locked.has('sections')){
      (c.sections||[]).forEach((s,i)=>{
        const ds=(d.sections||[]).find(x=>x?.section_key&&x.section_key===s.section_key)||(d.sections||[])[i];
        if(!ds)return;
        if(lang==='ar'){if(isBlank(s.title_ar))s.title_ar=ds.title_ar||s.title_ar;if(isBlank(s.body_ar))s.body_ar=ds.body_ar||s.body_ar;}
        else{if(isBlank(s.title))s.title=ds.title||s.title;if(isBlank(s.body))s.body=ds.body||s.body;}
      });
    }
    if(!locked.has('tags')){
      (c.tags||[]).forEach((tag,i)=>{
        const dt=(d.tags||[]).find(x=>x?.slug&&x.slug===tag.slug)||(d.tags||[])[i];if(!dt)return;
        if(lang==='ar'){if(isBlank(tag.name_ar))tag.name_ar=dt.name_ar||tag.name_ar;}
        else if(isBlank(tag.name))tag.name=dt.name||tag.name;
      });
    }
    if(!locked.has('guided_review')){
      c.guided_review=c.guided_review||{};
      deepLanguageFill(c.guided_review,d.guided_review||{},lang);
    }
    return c;
  }
  async function fillMissingLanguage(lang){
    const bad=invalidJsonField();if(bad)return toast(bad+'.');
    const p=syncPayloadFromForm(),source=p.source_text.trim();
    if(source.length<20)return toast('Source Content is required before Fill Missing EN/AR.');
    const button=lang==='ar'?$('#hsec-fill-ar'):$('#hsec-fill-en');
    button.disabled=true;button.textContent=lang==='ar'?'FILLING AR…':'FILLING EN…';
    try{
      const res=await fetch(cfg.supabaseUrl+'/functions/v1/hse-ai-content-builder',{
        method:'POST',
        headers:{'Content-Type':'application/json','apikey':cfg.supabaseKey,'Authorization':'Bearer '+cfg.supabaseKey,'x-portfolio-device-id':device.id,'x-portfolio-device-secret':device.secret},
        body:JSON.stringify({source_text:source,mode:lang==='ar'?'fill_missing_ar':'fill_missing_en',current:p,locked_fields:p.locked_fields})
      });
      const body=await res.json();if(!res.ok)throw new Error(body.error||'Fill Missing workflow failed');
      const merged=mergeMissingLanguage(p,body.data,lang);
      fillEditor(merged);
      $('#hsec-ai-result').innerHTML='<b>FILL MISSING '+(lang==='ar'?'AR':'EN')+' APPLIED TO EMPTY FIELDS ONLY.</b><br>Review every inserted value. Nothing was saved or published.';
      toast('Missing '+(lang==='ar'?'Arabic':'English')+' fields filled in editor only');
    }catch(e){toast(e.message)}
    finally{button.disabled=false;button.textContent=lang==='ar'?'FILL MISSING AR':'FILL MISSING EN'}
  }
