(() => {
  ...
  function startForm(){return `<article class="portal-card hse-start-card"><p class="eyebrow">${t('ONE CLEAR FIRST STEP','خطوة أولى واحدة وواضحة')}</p><h3>${t('Start with a professional HSE consultation.','ابدأ باستشارة HSE احترافية.')}</h3><p>${t('You do not need to choose a report, assessment or technical solution first. Tell us what is happening and Andrew will shape the right next step after the consultation.','مش مطلوب منك تختار تقرير أو تقييم أو حل فني من البداية. احكِ اللي بيحصل، وبعد الاستشارة Andrew يحدد الخطوة المناسبة.')}</p><form id="hse-start-form" class="hse-simple-form"><label><span>${t('MAIN HAZARD / ISSUE','الخطر / المشكلة الرئيسية')}</span><select id="hse-hazard" required>${hazardOptions()}</select></label><label><span>${t('WHAT IS HAPPENING?','إيه اللي بيحصل؟')}</span><textarea id="hse-summary" required minlength="10" placeholder="${esc(t('A short plain-language description is enough.','وصف بسيط بطريقتك العادية كفاية.'))}"></textarea><small>${t('No technical wording needed. You will not have to repeat this information later.','مش محتاج مصطلحات فنية، ومش هتحتاج تكرر البيانات دي بعد كده.')}</small></label><label><span>${t('COUNTRY / JURISDICTION · OPTIONAL','الدولة / جهة التطبيق · اختياري')}</span><input id="hse-jurisdiction" placeholder="${esc(t('Example: Egypt','مثال: مصر'))}" /></label><div class="hse-fee-box"><div><b>${t('Professional HSE Consultation · 60 minutes','استشارة HSE احترافية · 60 دقيقة')}</b><small>${t('Consultation only. Additional services are optional and quoted separately.','السعر للاستشارة فقط، وأي خدمات إضافية اختيارية ويتم تسعيرها بشكل منفصل.')}</small></div><strong>$75</strong></div><button class="portal-button" id="hse-start-button" type="submit">${t('START MY CONSULTATION →','ابدأ طلب الاستشارة ←')}</button></form></article>`}
  ...
  function render(){
    ...
    root.innerHTML=`<div class="hse-workspace" dir="${ar()?'rtl':'ltr'}"><div class="hse-view-head"><div><p class="eyebrow">INVESTIGATOR'S EYE</p><h2>${cases.length?t('Your HSE Workspace','مساحة HSE الخاصة بك'):t('HSE Consultation','استشارة HSE')}</h2><p>${t('One clear next action at a time. Everything else stays in the case record.','إجراء واحد واضح في كل مرحلة، وباقي التفاصيل تفضل محفوظة في سجل الحالة.')}</p></div><div class="hse-rule"><b>${t('$75 = consultation only.','75 دولار = الاستشارة فقط.')}</b><br>${t('Reports, assessments, surveys, site visits and technical deliverables are separate and optional.','التقارير والتقييمات والقياسات وزيارات الموقع والمخرجات الفنية خدمات منفصلة واختيارية.')}</div></div>${currentHtml}${historyHtml}${startHtml}</div>`;
  }
