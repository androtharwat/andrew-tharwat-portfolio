(() => {
  ...
  function recommendations(c,items){
    return `<div class="portal-card" style="margin-top:14px"><p class="eyebrow">RECOMMENDED FOR YOUR CASE</p><h3>Choose what you want us to handle.</h3><p>Nothing is added automatically. Tick only the services you want, then continue once.</p><div class="hse-service-choice">${items.map(s=>{const quoted=s.quoted_price!=null;const price=quoted?money(s.quoted_price,s.currency||'USD'):(s.studio_service_catalog?.pricing_mode==='quote'?'Quote required':`From ${money(s.studio_service_catalog?.display_price_usd||0)}`);return `<label><input type="checkbox" value="${s.id}" data-hse-service-choice /><div><b>${esc(s.studio_service_catalog?.name_en||'Service')}</b><small>${esc(s.recommendation_reason||'Recommended after consultation')}${quoted?' · Exact price ready':' · We will confirm the exact scope and price before work starts'}</small></div><strong>${esc(price)}</strong></label>`}).join('')}</div><p class="hse-service-note">Quoted items can move directly to one combined payment. Unquoted items only request an exact scope and quote — they are not purchased yet.</p><div class="hse-selected-action"><p>You can leave any item unchecked and decide later.</p><button type="button" data-hse-continue="${c.id}">CONTINUE WITH SELECTED SERVICES →</button></div></div>`}
  ...
  function render(){
    ...
    root.innerHTML=`<div class="hse-view-head"><div><p class="eyebrow">INVESTIGATOR'S EYE</p><h2>HSE Consultation</h2><p>One consultation first. After that, only the services your case actually needs.</p></div><div class="hse-rule"><b>$75 = consultation only.</b><br>Reports, assessments, surveys, site visits and technical deliverables are separate and optional.</div></div>${cases.length?`<div class="hse-case-stack">${cases.map(caseCard).join('')}</div><div style="margin-top:12px">${startForm()}</div>`:startForm()}`;
  }
